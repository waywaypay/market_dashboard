...
* **Quotes (Yahoo)** — no key needed. Two chart-API calls per ticker: trailing
  daily history for `avg_volume` + `sigma` (stdev of the last
  `QUOTES_TRAILING_DAYS` daily % moves, default 20), and today's tape with
  `includePrePost` for the latest pre-market trade, today's volume and the
  prior close. RVOL and unusual-move flags stay derived in the fuse stage. A
  failing symbol is skipped, not fatal; the pull only fails if every ticker
  does. The endpoint is public but unofficial — if it misbehaves, mix back
  with `BRIEF_QUOTES=fixture` while you pick a paid vendor.

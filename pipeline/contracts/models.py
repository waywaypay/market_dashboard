...
class DailyBrief(BaseModel):
    ...
    tldr: str  # one-line synthesis from the classifier (The Read band)
    counts: Counts
    ...
    # -- "Today's First Read": a short narrative morning note synthesizing the
    #    brief (the editorial expansion of the one-line `tldr`). Generated by the
    #    FirstReadProvider — VeniceAI when VENICE_API_KEY is set, otherwise a
    #    deterministic composer — so a keyless run still ships a note. It becomes
    #    the lede of the First Read email and its own dashboard band; `tldr` is
    #    untouched. Optional + defaulted so older artifacts validate, and empty
    #    means "no note this run" (the UI/email fall back to `tldr`).
    #    `first_read_engine` records provenance like `classifier_engine`. --
    first_read: str = ""
    first_read_engine: str = "none"  # "venice" | "fixture" | "none" (+ failure note)
